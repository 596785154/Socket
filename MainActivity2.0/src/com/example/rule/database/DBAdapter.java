...
public class DBAdapter {
	 ...
	 * 完成对数据库的增删改查
	 * 1.插入数据
	 * @param obj  插入对象（）  判断当前对象，插入到相应的数据表
	 * 
	 */
	//插入数据
	...
	public List<String> showUserName()
	{
		...
		open();
		Cursor cursor=mydb.query(p_plant, new String[]{"plantName"}, null, null, null, null, null);
		while(cursor.moveToNext())
		{
			String userNameOne=cursor.getString(0);
			userNameList.add(userNameOne);
		}
		close();
		return userNameList;
	}
	 ...
	 * @return 这种植物索要生长的信息
	 */
	public List<HashMap<String,String>> showOne(String _plantName)
	{
		...
		open();
		Cursor cursor=mydb.query(p_plant, new String[]{"plantType","plantName","tempuraturemin","tempuraturemax","humiditymin","humiditymax","yoututempuraturemin","yoututempuraturemax","youtuhumiditymin","youtuhumiditymax","atmospheremin","atmospheremax","lightmin","lightmax","co2min","co2max"}, "plantName=?", new String[]{_plantName}, null, null, null);
		while(cursor.moveToNext())
		{	
